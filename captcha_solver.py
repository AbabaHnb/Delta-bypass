...
def session():
    s = requests.Session()
    ad = requests.adapters.HTTPAdapter(pool_connections=8, pool_maxsize=32, max_retries=0)
    s.mount('https://', ad)
    s.mount('http://', ad)
    s.headers.update({'User-Agent': rand_ua(), 'Referer': 'https://captcha.platorelay.com/'})
    return s

# numba 加速

@njit(cache=True)
def label_centroids(mask, min_area=3):
    # 连通域标记 质心计算
    H, W = mask.shape
    #连通域标记
...
def blobs(mask, min_area=3):
    #numba加速
    return label_centroids(mask, min_area)

@njit(cache=True)
def label_full(mask, min_area=3):
    #连通域标记
    H, W = mask.shape
    labels = np.zeros((H, W), np.int32)
    eq = np.zeros(4096, np.int32)
...
def track_at_threshold(fr, th, min_area=10, match_r=35, grays=None):
    H, W = fr[0].shape[:2]
    if grays is None:
        grays = gray_frames(fr)
    fcent = []
    for g in grays:
        m = g < th
...
def region_centroid_rotation(fr, cx0, cy0, half=35, th=170, grays=None):
    H, W = fr[0].shape[:2]
    if grays is None:
        grays = gray_frames(fr)
    centroids = []
...
def grid_angular_momentum(fr, grid=6, th=170, grays=None):
    #角动量扫描
    H, W = fr[0].shape[:2]
    if grays is None:
        grays = gray_frames(fr)
...
def grid_find_opposite_from_mom(norm_mom, pixel_count, major_sign, fr, grays, grid=6, th=170):
    #角动量里找反向旋转最强的块
    H, W = fr[0].shape[:2]
    bh, bw = H // grid, W // grid
...
def grid_find_opposite(fr, major_sign, grid=6, th=170, grays=None):
    #角动量中找反向旋转最强的块 返回该块内暗像素的精确质心
    norm_mom, pixel_count = grid_angular_momentum(fr, grid, th, grays=grays)
    H, W = fr[0].shape[:2]
    bh, bw = H // grid, W // grid
...
def track_bgsub(fr, delta=20, min_area=40, match_r=35, grays=None):
    #背景差分追踪
    H, W = fr[0].shape[:2]
    if grays is None:
        grays = gray_frames(fr)
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        futures = {pool.submit(label_full, (bg - g) > delta, min_area): i for i, g in enumerate(grays)}
...
def bgsub_minority_from_tracks(tracks):
    #从bgsub轨迹中找清晰图形
...
def bgsub_find_minority(fr, grays=None):
    #背景差判定
    try:
        tracks = track_bgsub(fr, delta=12, min_area=15, match_r=35, grays=grays)
    except Exception:
        return None
    return bgsub_minority_from_tracks(tracks)

def driftodd_predict_v8(fr):
    V8_DEBUG.clear()
    V8_DEBUG['frames'] = len(fr)
    H, W = fr[0].shape[:2]
    ...
    #用背景差分判断
    try:
        bg_tracks = track_bgsub(fr, delta=12, min_area=15, match_r=35, grays=grays)
    except Exception:
        bg_tracks = []
    bg_tgt = bgsub_minority_from_tracks(bg_tracks)
    if bg_tgt is not None:
        V8_DEBUG['selected'] = ('bgsub_minority', bg_tgt['cx'], bg_tgt['cy'],
                                 bg_tgt['total_disp'])
        return bg_tgt['cx'], bg_tgt['cy']

    #角动量fallback
    norm_mom, pixel_count = grid_angular_momentum(fr, grid=6, th=170, grays=grays)
    if pixel_count.max() >= 10:
        major_sign = 1 if norm_mom.sum() >= 0 else -1
        gx, gy, gval = grid_find_opposite_from_mom(
            norm_mom, pixel_count, major_sign, fr, grays, grid=6, th=170)
        if gx is not None:
            V8_DEBUG['selected'] = ('grid_primary', gx, gy, gval)
            return gx, gy

    # 阈值追踪
    all_results = []
    with ThreadPoolExecutor(max_workers=4) as pool:
        futures = {pool.submit(track_at_threshold, fr, th, 10, 35, grays): th
    ...
    V8_DEBUG['pos'] = len(pos)
    V8_DEBUG['neg'] = len(neg)
    ...
    if len(pos) != len(neg) and min(len(pos), len(neg)) > 0:
        ...
        if real_minority:
            ...
            V8_DEBUG['selected'] = ('minority', tgt['cx'], tgt['cy'], tgt['total_disp'])
    ...
    if opposite:
        ...
        V8_DEBUG['selected'] = ('opposite_disp', best['cx'], best['cy'], best['total_disp'])
        return best['cx'], best['cy']

    #低阈值重扫
    low_results = []
    with ThreadPoolExecutor(max_workers=2) as pool:
        futures = {pool.submit(track_at_threshold, fr, th, 8, 35, grays): th
                   for th in [80, 90]}
        for fut in as_completed(futures):
            try:
    ...
    if low_opposite:
        ...
        V8_DEBUG['selected'] = ('low_th_opposite', best['cx'], best['cy'], best['total_disp'])
        return best['cx'], best['cy']
    ...
    if bg_dedup:
        best = bg_dedup[0]
        V8_DEBUG['selected'] = ('bgsub_opposite', best['cx'], best['cy'], best['total_disp'])
        return best['cx'], best['cy']

    #角动量扫描
    gx, gy, gval = grid_find_opposite(fr, major_sign, grays=grays)
    V8_DEBUG['grid_angmom'] = (gx, gy, gval)
    if gx is not None:
        V8_DEBUG['selected'] = ('grid_opposite', gx, gy, gval)
        return gx, gy
    ...
    if len(rots) >= 3:
        ...
        if outliers:
            ...
            V8_DEBUG['selected'] = ('area_outlier', best['cx'], best['cy'], best['area'])
            return best['cx'], best['cy']
    ...
    if weak:
        ...
        V8_DEBUG['selected'] = ('weak_same', best['cx'], best['cy'])
        return best['cx'], best['cy']

    tgt = max(rots, key=lambda r: r['conf'])
    V8_DEBUG['selected'] = ('fallback_major', tgt['cx'], tgt['cy'])
    return tgt['cx'], tgt['cy']
...
def coherence_predict(fr):
    ...
    if np.isfinite(valid).any():
        ...
        if len(ys) > 0:
            ...
            V8_DEBUG['selected'] = ('coherence_centroid', x, y, float(rmin))
            return min(max(x, 25), W - 25), min(max(y, 25), H - 25)
    # 回退coarse格中心
    x = min(max(i * bw + bw / 2.0, 25), W - 25)
    y = min(max(j * bh + bh / 2.0, 25), H - 25)
    V8_DEBUG['selected'] = ('coherence_coarse', x, y, float(rate[j, i]))
    return x, y
